    ...
    methods: {
        fetchData(page = 1) {
            ...
            if (this.createTimes != null & this.createTimes.length == 2) {
                this.searchObj.createTimeBegin = this.createTime[0]
                this.searchObj.createTimeEnd = this.createTime[1]
            }
            api.getPageList(page, this.limit, this.searchObj).then(response => {
        ...
        edit(id) {
            api.getUserById(id).then(response => {
                ...
                this.sysUser.name = response.data.name
                this.sysUser.phone = response.data.phone
                this.sysUser.status = response.data.status
            })
            this.dialogVisible = true
        },
        switchStatus(row) {
